...
| R-012 | `waehrung` ist ein gültiger ISO-4217-Code | anfrage | G1/B2/C1 | HART | RD | ISO 4217 |
...
| R-029 | numerischer Teil von `sf_klasse_hp` ≤ Alter(VN) − 17 | risiko_kfz + person | G2/B4/C1 | HART | FAN CFD; CHU | Man kann nicht länger schadenfrei fahren, als man den Führerschein besitzt |
| R-030 | `sf_klasse_vk` ≤ `sf_klasse_hp` (numerisch, wenn beide numerisch) | risiko_kfz | G2/B4/C2 | WARNUNG | FAN | Marktüblich; Ausnahmen existieren, daher Warnung |
...
| R-041 | Genau eines von `sb_hausrat_prozent` und `sb_hausrat_eur` ist gefüllt | angebot | G2/B4/C2 | WARNUNG | RD; KIM | **Modellannahme des Schemas**, kein Domänenfakt — reale Produkte kennen kombinierte Formen („10 %, mind. 500 €, max. 2.500 €") |
...
| R-053 | `zahlbeitrag_rate_eur` liegt je Sparte im plausiblen Korridor (Kfz: 40 – 6.000 €/Jahr; Hausrat: 20 – 2.000 €/Jahr) | angebot | G5/B3/C2 | WARNUNG | ABE Outliers; FOI | Werte weit außerhalb deuten auf Cent-statt-Euro-Interpretation hin — Ursache: implizite Dezimalstellen im GDV-Format („10,2" bedeutet 10 Vor- und 2 Nachkommastellen ohne Trennzeichen) |
...
| R-057 | Das Pflichtfeldprofil ist je `quell_schnittstelle` eingehalten (BIPRO_420 und BIPRO_RNEXT verlangen ein höheres Niveau als CSV_IMPORT) | angebot + anfrage | G5/B1/C2 | WARNUNG | RD Multi-Source; DAMA Completeness | Versicherer befüllen dasselbe Feld unterschiedlich tief; das Pflichtfeldprofil steht in `spec/01_datenmodell.md`, Abschnitt 5 |
