...
| R-009 | Jedes Datumsfeld auf der Rohdatenschicht ist ein existierender Kalendertag. **Geprüft werden die Felder vom Typ `DATUM`** (Rohform `TTMMJJJJ`), nicht die beiden Zeitpunktfelder `eingangszeitpunkt` und `berechnungszeitpunkt` (ISO 8601) | alle (Rohschicht `df_raw`) | G1/B2/C1 | HART | RD; FOI | `31022026` ist formal achtstellig, aber kein Kalendertag. Format und Kalendergültigkeit werden getrennt geprüft. **Diese Regel arbeitet zwingend auf der Rohdatenschicht** — auf der typisierten Schicht ist sie per Konstruktion nicht verletzbar (siehe `spec/01`, Abschnitt 6). Ein Zeitpunkt ist im Datenmodell ein eigener Typ mit eigener Serialisierung; ihn mitzuprüfen würde die Regel verbreitern, ohne dass der Katalog es verlangt |
...
| R-032 | `versicherungsteuer_eur` = ROUND_HALF_UP(`netto` × `satz` / 100, 2), **Toleranz wie R-031** | angebot | G2/B4/C1 | HART | FAN | § 6 i. V. m. § 5 VersStG. Die Regel verwendet denselben Konfigurationswert wie R-031 (`schwellen.r031_toleranz_eur`). Ein eigener Schlüssel wurde bewusst **nicht** angelegt: Beide Regeln betreffen denselben Rundungsschritt, und zwei Schwellen dafür könnten auseinanderlaufen, ohne dass es auffiele |
...
| ID | Prädikat (muss erfüllt sein) | Geltungsbereich | A/B/C | Schwere | Literatur | Fachliche Grundlage |
|---|---|---|---|---|---|---|
| **R-052** | Innerhalb einer `anfrage_id` verwenden alle Angebote dieselbe Einheitenkonvention für den Selbstbehalt (entweder alle Prozent oder alle Betrag). **Gemeldet wird die Minderheit**; bei Gleichstand alle beteiligten Angebote | angebot | G5/B7/C2 | WARNUNG | RD „Different value representations"; KIM 2.2.3; FOI Consistency; DAMA Consistency | Anbieter A liefert den Selbstbehalt in Euro, Anbieter B in Prozent — der Vergleich wird dadurch unzulässig. Welche der beiden Konventionen die richtige ist, sagt keine Norm; die innerhalb der Anfrage überwiegende ist der einzige verfügbare Bezugspunkt. Diese Heuristik ist der Grund für C2 und WARNUNG |
...
| R-054 | Kein Angebot einer Anfrage weicht um näherungsweise den Faktor 12 (± 5 %) vom Median der übrigen Angebote derselben Anfrage ab. **Geprüft wird nur, wenn mindestens zwei Vergleichsangebote vorliegen** (also ab drei bepreisten Angeboten je Anfrage) | angebot | G5/B7/C2 | WARNUNG | RD; FOI Consistency | Monats- statt Jahresbeitrag. **Wichtig:** Diese Verwechslung ist ein Faktor 12 und deshalb nicht über eine absolute Untergrenze zu finden, sondern nur relational. Bei genau zwei bepreisten Angeboten wäre der „Median der übrigen" das jeweils andere Angebot; beide Seiten sähen füreinander wie der Fehler aus, die Regel müsste beide melden und käme bestenfalls auf eine Precision von 0,5. Das kostet Recall bei Anfragen mit zwei bepreisten Angeboten — die Zahl ist in der Auswertung zu berichten |
...
| davon HART | 47 |
| davon WARNUNG | 11 |
| C1 (deterministisch) | 44 |
| C2 (heuristisch) | 11 |
| C3 (referenzdatenabhängig) | 3 (R-050, R-051, R-058) |
